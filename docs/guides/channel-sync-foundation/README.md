...
2. 标签双向：远端改动可入库，本地改动可回写。
   - 企业客户标签页可编辑“客户-标签”关系（客户侧/标签侧）。
   - 编辑后先进入待回写变更，再提交同步中心 `pushback` 任务执行。
3. 组织双向：部门/成员能拉取并回写。
4. 外部联系人与线索双向：
   - 能拉取外部联系人入池。
   - 能按白名单回写线索。
...
### Q4: 企业客户标签页面看起来改了，但企业微信没变化

1. 先确认是否点击了“提交回写任务”（仅编辑不会自动写远端）。
2. 到标签同步中心确认出现 `direction=push`、`mode=pushback` 任务。
3. 查看任务结果总结中的 `回写` 与 `冲突` 数；冲突不为 0 时去冲突队列重放。
