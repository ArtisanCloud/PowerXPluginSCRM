...
## 同步基础域门禁状态（2026-04-08）

- 状态：`Blocked by Sync Foundation`
- 约束：在 `006-channel-sync-foundation` 前置门禁全部达成前，渠道活码仅允许缺陷修复，不新增业务能力。
- 解锁条件：以 `docs/plan/develop/channel-sync-foundation/README.md` 的门禁清单为唯一准入标准。
