...
### V2 页面对标补充（2026-04-29）

1. 员工活码设置页对齐业务态交互：
   - 双栏布局（左配置右预览）；
   - 成员/标签改为结构化选择器，不再主流程手填 ID；
   - 回复设置支持文本、图片、链接、小程序、雷达关联入口。
2. 保存与发布动作明确分离：
   - 保存仅写本地；
   - 发布触发渠道同步并展示状态/错误。
3. 修复交互稳定性：
   - “添加回复内容”浮层定位与层级问题（避免错位与裁切）；
   - 在常见桌面分辨率与 125% 缩放下可用。
...
## V2.1 Implementation Alignment (2026-04-19)

### Scope (群运营闭环)

- 群活码：从骨架升级为全量（入群方式 CRUD + 发布状态机）。
- 群聊同步：全量（`groupchat/list + groupchat/get + 客户群变更回调`）。
- 本地群标签：全量（标签定义、绑定关系、自动打标规则）。
- 群运营分析：全量（筛选、统计、导出）。

### Backend Changes (V2.1)

1. 群活码服务实装：
   - 接入 WeCom 入群方式 API；
   - 建立发布同步状态与重试链路；
   - 持久化 `config_id/state/qr_code`。
2. 群聊同步服务实装：
   - 周期任务：群列表拉取 + 详情补全；
   - 回调任务：客户群变更事件增量更新；
   - 幂等与补偿机制。
3. 本地群标签服务新增：
   - `group_tag_definitions` / `group_tag_bindings`；
   - 自动规则引擎（来源活码/群主/成员结构）。
4. 运营查询与导出：
   - 统一查询口径与导出口径；
   - 增加来源活码维度聚合统计。
5. 回调增量补打标：
   - 自建应用回调与代开发 openwork 回调分别验签解密；
   - 统一归一为客户群事件 envelope，并复用同一业务分发；
   - 对新增入群成员执行 `chat_id + external_userid` 增量企业标签补打。

### Frontend Changes (V2.1)
...
### Rollout Strategy (V2.1)

- 采用 feature flag：`acquisition.group_ops_v21`。
- 分三步灰度：
  1) 仅开放群活码发布与状态；
  2) 开放群同步与群列表；
  3) 开放群标签规则与运营分析导出。
