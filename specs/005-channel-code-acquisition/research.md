...
## V2.1 决策补充（2026-04-19）

## Decision 11: 群运营闭环采用“活码优先”推进顺序
- **Decision**: 先实装群活码（入群方式）再做群同步、群标签、群分析。
- **Rationale**: 群活码是来源归因入口，后续筛选与分析依赖 `config_id/state`。
- **Alternatives considered**:
  - 先做群标签：可快速有页面，但来源追因断裂，运营价值不足。

## Decision 12: 群标签定义为“本地运营标签”
- **Decision**: 群标签只在平台本地存储和计算，不回写企业微信官方群标签。
- **Rationale**: 企微官方未提供可稳定写入“客户群标签”能力；本地标签可先满足运营分群与分析。
- **Alternatives considered**:
  - 强行映射到企微字段：接口能力不稳定且易误导用户。

## Decision 13: 群同步采用“拉取 + 回调”双通道最终一致
- **Decision**: 周期拉取群列表/详情 + 回调增量更新并行，按 `chat_id` 合并快照。
- **Rationale**: 回调可能丢失，拉取可能延迟；双通道能提高完整性与时效性。
- **Alternatives considered**:
  - 仅回调：覆盖不完整；
  - 仅拉取：实时性不足。

## V2.1 风险补充（2026-04-19）

## Risk 1: 群标签规则当前为最小实现，命中语义需二次收敛
- **现状**: 规则重放与绑定流程已打通，支持手工重放和执行记录落库。
- **风险**: “来源活码/群主/成员结构”的复杂规则 DSL 尚未完全产品化，跨团队理解成本较高。
- **缓解建议**:
  - 在 `rule_payload` 增加版本字段并固定 schema；
  - 为规则命中结果补充 explain 字段（命中原因）。

## Risk 2: 导出能力当前以前端 CSV 为主，后端导出口径尚未标准化
- **现状**: 群分析页可导出当前筛选结果；后端独立 `/export` 接口未开放。
- **风险**: 大数据量场景下前端导出稳定性与审计追踪不足。
- **缓解建议**:
  - 下一阶段补后端异步导出任务；
  - 导出任务记录筛选条件快照与触发人。

## Risk 3: 渠道侧真实 API 适配仍需灰度验证
- **现状**: 群活码发布状态机、群聊回调入站和本地落库链路已可运行。
- **风险**: 不同租户渠道账号配置差异（权限、回调开关）会导致环境行为不一致。
- **缓解建议**:
  - 在运行手册中固化“账号权限 + 回调连通 + 默认账号”三项验收清单；
  - 增加按 `channel_account_uuid` 的失败率观测与告警阈值。
