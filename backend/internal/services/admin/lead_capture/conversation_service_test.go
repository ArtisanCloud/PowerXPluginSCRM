...
func TestConversationService_IdempotencyAndDuplicateWebhook(t *testing.T) {
	...
	svc := NewConversationService(
		...
		WithLeadService(NewLeadService(leadrepo.NewLeadRepository(db))).
...
func TestConversationService_AutoCreateLeadRuleToggle(t *testing.T) {
	db := openConversationServiceTestDB(t, "conversation_service_auto_create_toggle")
	tenantUUID := "00000000-0000-0000-0000-000000000001"
	...
	// Default (rule disabled): should go to pending and not create lead.
	_, created, err := svc.IngestWebhook(context.Background(), ConversationWebhookInput{
		TenantUUID:         tenantUUID,
		ChannelAccountUUID: accountUUID,
		ExternalEventID:    "evt-toggle-1",
		ConversationID:     "conv-toggle-1",
		ActorType:          "customer",
		ActorID:            "customer-1",
		Direction:          "inbound",
		MessageType:        "text",
	...
	require.NoError(t, db.Model(&leadmodel.LeadConversationPending{}).Where("conversation_id = ?", "conv-toggle-1").Count(&pendingCount).Error)
	require.EqualValues(t, 1, pendingCount)

	// Enable rule and ingest another customer message: should auto create lead.
	_, err = leadrepo.NewChannelRuleRepository(db).UpsertAutoCreateLeadRule(context.Background(), tenantUUID, "wechat", "wecom", true)
	require.NoError(t, err)
	_, created, err = svc.IngestWebhook(context.Background(), ConversationWebhookInput{
		TenantUUID:         tenantUUID,
		ChannelAccountUUID: accountUUID,
		ExternalEventID:    "evt-toggle-2",
		ConversationID:     "conv-toggle-2",
		ActorType:          "customer",
		ActorID:            "customer-2",
		Direction:          "inbound",
		MessageType:        "text",
	...
	require.NoError(t, db.Model(&leadmodel.LeadConversationBinding{}).Where("conversation_id = ?", "conv-toggle-2").Count(&bindingCount).Error)
	require.EqualValues(t, 1, bindingCount)
	require.NoError(t, db.Model(&leadmodel.LeadConversationPending{}).Where("conversation_id = ?", "conv-toggle-2").Count(&pendingCount).Error)
	require.EqualValues(t, 0, pendingCount)
}
