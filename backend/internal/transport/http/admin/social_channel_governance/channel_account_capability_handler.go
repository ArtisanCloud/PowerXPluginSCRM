...
type ChannelAccountCapabilityHandler struct {
	svc *SocialService.ChannelAccountCapabilityService
}

func NewChannelAccountCapabilityHandler(svc *SocialService.ChannelAccountCapabilityService) *ChannelAccountCapabilityHandler {
	return &ChannelAccountCapabilityHandler{svc: svc}
}

type channelAccountCapabilityRequest struct {
	Capabilities map[string]bool `json:"capabilities" binding:"required"`
}

func (h *ChannelAccountCapabilityHandler) UpdateChannelAccountCapabilities(c *gin.Context) {
	if h.svc == nil {
		contracts.ResponseServiceUnavailable(c, "channel account capability service unavailable", nil)
		return
	}
	var req channelAccountCapabilityRequest
	...
	account, err := h.svc.UpdateChannelAccountCapabilities(c.Request.Context(), tenantUUID, accountUUID, req.Capabilities)
	if err != nil {
		switch {
