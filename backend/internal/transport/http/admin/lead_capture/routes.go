...
func RegisterRoutes(rg *gin.RouterGroup, deps *app.Deps) {
	...
	group := rg.Group("/leads", httpmw.EnsureTenant())
	{
		...
		group.GET("/:lead_id/timeline", handler.ListTimeline)
		group.POST("/:lead_id/activities", handler.RecordActivity)
		group.GET("/:lead_id/node-attachments", handler.ListNodeAttachments)
		group.POST("/:lead_id/node-attachments", handler.UploadNodeAttachment)
		...
		group.DELETE("/:lead_id/attachments/:attachment_id", handler.DeleteAttachment)
