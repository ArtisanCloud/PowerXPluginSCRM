...
export const usePermissionStore = defineStore("permission", () => {
  ...
  const fetchRolePermissionIDs = async (roleId: number) => {
    const res = await get<any>(`${baseUrl}/roles/${roleId}`);
    const payload = res?.data?.data ?? res?.data ?? res;
    const ids = Array.isArray(payload?.permission_ids)
      ? payload.permission_ids.map((id: any) => Number(id))
      : [];
  ...
  const setRolePermissionIDs = async (
    roleId: number,
    tenantUuid: string,
    ids: number[]
  ) => {
    if (!tenantUuid) {
      throw new Error("tenant_uuid is required when设置角色权限");
    }
    const payload = {
      tenant_uuid: tenantUuid,
      permission_ids: ids,
    };
    const res = await put<any>(`${baseUrl}/roles/${roleId}/permissions`, payload);
    const data = res?.data?.data ?? res?.data ?? res;
    const now: number[] = Array.isArray(data?.permission_ids)
      ? data.permission_ids.map((id: any) => Number(id))
      : ids;
